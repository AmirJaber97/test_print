...
class PrinterAdapter(var sunmiPrinterService: SunmiPrinterService?, var resources: Resources) {
    ...
    fun getStrList1(inputString: String, length: Int,
                    size: Int): List<String>? {
        val list: MutableList<String> = ArrayList()
        for (index in 0 until size) {
            val childStr: String = substring1(inputString, index * length,
                    (index + 1) * length)
            list.add(childStr)
        }
        return list
    ...
    private fun printGoods(menuBean: InvoiceDto, fontsizeContent: Int, divide2: String, payMode: Int, width: Int) {
        ...
        val real: String = "Cash:"
        val vat: String = "Vat:"

        blank1 = width * 2 - total.length -( menuBean.total?.length ?:0)
        blank2 = width * 2 - real.length - (menuBean.cash?.length ?:0)
        blank3 = width * 2 - vat.length - (menuBean.vat?.length ?:0)
...
//        sunmiPrinterService?.printTextWithFont("مؤسسة كوب و قطة للحيوانات الاليفة", "", 28f, printerCallBack)
    ...
    fun addProductPrintLine(title: String, q: String, price: String) {
        sunmiPrinterService?.lineWrap(1, printerCallBack)
        sunmiPrinterService?.setAlignment(0, printerCallBack)
        sunmiPrinterService?.printTextWithFont(title, "", 22F, printerCallBack)
        sunmiPrinterService?.lineWrap(1, printerCallBack)
        sunmiPrinterService?.setAlignment(2, printerCallBack)
        sunmiPrinterService?.printTextWithFont(" x$q ", "", 20F, printerCallBack)
        sunmiPrinterService?.setAlignment(2, printerCallBack)
        sunmiPrinterService?.printTextWithFont(price, "", 24F, printerCallBack)
        sunmiPrinterService?.lineWrap(1, printerCallBack)
        sunmiPrinterService?.setAlignment(1, printerCallBack)
